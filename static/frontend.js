$(function() {
	...
	connection.onmessage = function(msg){
		data=msg.data
		if (data.slice(0,3) == "id="){
			id = data.slice(3);
			$("#idd").html('your id is '+id);
		} else if (data.slice(0,3) == "ERR"){
			$('#history').append('<div class="entry error">'+data+'</div>');
		} else if (data.slice(0,id.length) == id){
			$('#history').append('<div class="entry own">'+data+'</div>');
		} else {
			$('#history').append('<div class="entry">'+data+'</div>');
		}
		var count = $("#history div").length;
		if (count > 10) $("#history div").first().remove();
	}
